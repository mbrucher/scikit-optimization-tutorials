...
mystep = step.BFGSNewtonStep(np.eye(2))
...
plt.legend()
plt.title("Rosenbrock calls for the BFGS descent")
plt.savefig("ros_bfgs_calls.png")

plt.figure()
plt.semilogy(recorder.old, label="Old value")
plt.semilogy(np.arange(-1, -1 + len(recorder.new)), recorder.new, label="New value")
plt.legend()
plt.title("Rosenbrock values with the BFGS descent")
plt.savefig("ros_bfgs_values.png")
...
plt.title("Rosenbrock positions with the BFGS descent")
plt.savefig("ros_bfgs.png")
